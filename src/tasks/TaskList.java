...
public class TaskList implements Encodable {
    ...
    public TaskList(List<Task> tasks) {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            ...
            switch (tasks.get(i).getType()) {
                case TAKEOFF:
                    if (tasks.get(priorIndex).getType() != TaskType.LOAD) {
                        throw new IllegalArgumentException();
                    }
                    break;
                case AWAY:
                    if (!(tasks.get(priorIndex).getType() != TaskType.TAKEOFF
                            || tasks.get(priorIndex).getType() != TaskType.AWAY)) {
                        throw new IllegalArgumentException();
                    }
                    break;

                case LAND:
                    if (tasks.get(priorIndex).getType() != TaskType.AWAY) {
                        throw new IllegalArgumentException();
                    }
                    break;
                case LOAD:
                case WAIT:
                    if (!(tasks.get(priorIndex).getType() != TaskType.LAND
                            || tasks.get(priorIndex).getType() != TaskType.AWAY)) {
                        throw new IllegalArgumentException();
                    }
                    break;
    ...
    public String encode() {
        ...
        for (int i = 0; i < this.tasks.size(); i++) {
            encodedTaskList.append(",");
            encodedTaskList.append(this.tasks.get(this.currentTaskIndex + i).encode());

        }
        encodedTaskList.replace(0, 1, "");
