 ...
 * @ass1
 */
public abstract class Terminal implements Encodable, EmergencyState, OccupancyLevel {
    /**
    ...
    public boolean equals(Object obj) {
        ...
        return terminalNumber == terminal.terminalNumber && emergency
                == terminal.emergency && gates.equals(terminal.gates);
    }
    ...
    public String encode() {
        StringBuilder encodedTerminal = new StringBuilder();
        for (int i = 0; i < this.getGates().size(); i++) {
            encodedTerminal.append(this.getGates().get(i).encode());
            encodedTerminal.append("\n");
        }
        encodedTerminal.replace(encodedTerminal.length() - 1,
                encodedTerminal.length(), "");
        return String.format("%s:%d:%s:%d\n%s",
                getClass().getSimpleName(),
                getTerminalNumber(),
                hasEmergency(),
                getGates().size(),
                encodedTerminal.toString());
    }
}
