...
public class ViewModel {
    ...
    private final StringProperty suitableGateText = new SimpleStringProperty("");

    /** The currently selected (clicked) aircraft */
    private final ObjectProperty<Aircraft> selectedAircraft = new SimpleObjectProperty<>();

    /** The aircraft currently landing (i.e. just went from LAND to WAIT/LOAD) */
    private final ObjectProperty<Aircraft> aircraftLanding = new SimpleObjectProperty<>();

    /** The aircraft currently taking off (i.e. just went from TAKEOFF to AWAY) */
    private final ObjectProperty<Aircraft> aircraftTakingOff = new SimpleObjectProperty<>();
     ...
     * {@link ControlTowerInitialiser#createControlTower(Reader, Reader, Reader, Reader)}
     * @requires filenames != null &amp;&amp; filenames.size() == 4
     * @given
     */
    public ViewModel(List<String> filenames) throws IOException, MalformedSaveException {
    ...
    public EventHandler<ActionEvent> getDroneAlertHandler() {
        return actionEvent -> {
            for (Terminal terminal : getControlTower().getTerminals()) {
    ...
    public EventHandler<ActionEvent> getFindSuitableGateHandler() {
        return actionEvent -> {
            ...
            if ((getSelectedAircraft().getValue()
                    .getTaskList().getCurrentTask().getType()
                    == TaskType.LAND)) {
                return;
            }
            //try to find a gate, otherwise return "NoSuitableGateException" text in the GUI
            try {
                Gate gateToFind =
                        getControlTower().findUnoccupiedGate(getSelectedAircraft().getValue());
                getSuitableGateText().setValue(gateToFind.toString());
            } catch (NoSuitableGateException noSuitableGateException) {
                getSuitableGateText().setValue("NoSuitableGateException");
            }
        };
    }
     ...
     * @param queuesWriter writer to which the takeoff/landing queues and loading map will be
     *                     written
    ...
    public void saveAs(Writer tickWriter, Writer aircraftWriter, Writer queuesWriter,
        ...
        //writing the format for the ticks to be saved as
        tickWriter.write("" + (int) getControlTower().getTicksElapsed() + "");
        tickWriter.flush();
        //flushing the writer

        //writing the format for the aircrafts to be saved as
        if (getControlTower().getAircraft().isEmpty()) {
            aircraftWriter.write("" + 0 + "");
        } else {
            StringBuilder listOfAircraft = new StringBuilder();
            String numAircraftString = ("" + getControlTower().getAircraft().size() + "");
            listOfAircraft.append(numAircraftString);

            for (Aircraft singleAircraft : getControlTower().getAircraft()) {
                listOfAircraft.append("\n");
                listOfAircraft.append(singleAircraft.encode());
            }
            String aircraftFileWrite = listOfAircraft.toString();
            aircraftWriter.write(aircraftFileWrite);
        }
        //flushing the writer after writing for both cases
        aircraftWriter.flush();

        //writing the format for the queues to be saved as
        if (getControlTower().getLoadingAircraft().isEmpty()) {
            String queuesNoLoad = String.format("%s\n%s\nLoadingAircraft:0",
                    getControlTower().getTakeoffQueue().encode(),
                    getControlTower().getLandingQueue().encode());
            queuesWriter.write(queuesNoLoad);
        } else {
            //Build the appropriate representation of loading tasks
            StringBuilder queueWithLoad = new StringBuilder();
            queueWithLoad.append("\n").append(getControlTower()
                    .getTakeoffQueue().encode());

            queueWithLoad.append("\n").append(getControlTower()
                    .getTakeoffQueue().encode());

            queueWithLoad.append("LoadingAircraft:")
                    .append(getControlTower().getLoadingAircraft().size());

            //checking the loading aircraft and appending each entry
            for (Map.Entry<Aircraft, Integer> singleEntry
                    : getControlTower().getLoadingAircraft().entrySet()) {
                queueWithLoad.append("\n");

                queueWithLoad.append(singleEntry.getKey())
                        .append(":").append(singleEntry.getValue());
                queueWithLoad.append(",");

            }
            int lastIndexOfStringBuilder = queueWithLoad.lastIndexOf(queueWithLoad.toString());

            //removing the last comma (,) from the string builder
            queueWithLoad.replace(lastIndexOfStringBuilder - 1,
                    lastIndexOfStringBuilder, "");
            queuesWriter.write(queueWithLoad.toString());
        }
        //flushing the writer after writing for both cases
        queuesWriter.flush();

        //writing the format for the terminals with gates to be saved as
        if (getControlTower().getTerminals().isEmpty()) {
            terminalsWithGatesWriter.write("" + 0 + "");
        } else {
            StringBuilder listOfTerminals = new StringBuilder();
            String numTerminalsString = ("" + getNumTerminals().getValue() + "");
            listOfTerminals.append(numTerminalsString);
            for (Terminal singleTerminal : getControlTower().getTerminals()) {
                listOfTerminals.append("\n");
                listOfTerminals.append(singleTerminal.encode());
            }
            String terminalsFileWrite = listOfTerminals.toString();
            terminalsWithGatesWriter.write(terminalsFileWrite);
        }
        //flushing the writer after writing for both cases
    ...
    public void tick() {
        ...
        if (selectedAircraft.isNotNull().get()) {
            this.aircraftInfoText.set(generateAircraftInfoText(selectedAircraft.get()));
        }
    ...
    private void updateTakeoffLandAircraft() {
        ...
        for (Aircraft aircraft : getControlTower().getAircraft()) {
            ...
            if (currentTaskType == TaskType.AWAY && allTakeoffAircraft.contains(aircraft)) {
    ...
    private void fillTakeoffLandAircraftLists() {
        this.allTakeoffAircraft = findAircraftWithTask(getControlTower().getAircraft(),
                TaskType.TAKEOFF);
        this.allLandAircraft = findAircraftWithTask(getControlTower().getAircraft(),
                TaskType.LAND);
    }

    /* Returns all aircraft in the given list whose current task's type is the given type */
    private List<Aircraft> findAircraftWithTask(List<Aircraft> aircraft, TaskType taskType) {
        return aircraft.stream()
                .filter(a -> a.getTaskList().getCurrentTask().getType() == taskType)
    ...
    private String generateAircraftInfoText(Aircraft aircraft) {
        ...
        lineJoiner.add("Model:          \t" + aircraft.getCharacteristics().name());
    ...
    private String generateLoadingInfoText() {
        Map<Aircraft, Integer> loadingAircraft = this.getControlTower().getLoadingAircraft();
        ...
        for (Map.Entry<Aircraft, Integer> entry : loadingAircraft.entrySet()) {
            ...
            joiner.add(aircraft.getCallsign() + " at gate "
                    ...
                    + " ticks remaining (currently at " + aircraft.calculateOccupancyLevel()
                    + "%, will load to " + aircraft.getTaskList().getCurrentTask().getLoadPercent()
                    + "%)");
        }
